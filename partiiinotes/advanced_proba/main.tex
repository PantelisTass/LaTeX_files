...
\begin{theorem}[Kolmogorov's continuity criterion]\label{thm: kolmogorov continuity criterion}
...
\begin{proof}
	...
	\underline{$ 2,3\implies 4$:} $ 0 = \mu(\partial A) = \mu(A\setminus \int{A})$, hence $ \mu(\overline{A}) = \mu(A) = \mu(\int{A})$.\ \ 
	\underline{$ 2:$} $ \displaystyle\liminf_{n}\mu(\int{A})\geq \mu(\int{A})=\mu(A)$.
	...
	\]
	since $ f$ is continuous and $ \{f>t\}$ is open and $ \subset \int\{f\geq t\}$. Also observe thath there exists an at most countable number of $ t$ such that $ \mu(f = t)>0$. Thus, 
	\[
